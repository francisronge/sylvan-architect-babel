...
### Deeper Sol frame audit, 15 September

Francis challenged the first visual review, including Minimalism's opening
`Select C[Q]`. This follow-up audits the same saved outputs at `a9c1571`; it
makes no provider call and changes no product behavior or authored record.
The earlier pass was too narrow to establish dependable Replay. Capturing every
frame, checking clipping and checking the camera transform did not establish
that each named action happened, that current pronunciation was correct, or that
unchanged syntax retained its position.

Coverage: all 78 saved frame images were inspected again; all 78 frames were
also traversed sequentially through the production Replay controls at 1280×720,
with rendered frame IDs, visible label bounds, ink, panel text and camera
transforms read from the DOM. All 23 authored relations were compared with
their anchors, prior anchors, values, recovered claims and structural changes.
The 1600×1100 captures from the first pass provide the larger desktop comparison.
No browser warning or error was observed. This is a bounded audit of these two
outputs, not a guarantee about every derivation, viewport or interaction.

| Finding | Evidence and responsibility | Recommended response; not implemented |
| --- | --- | --- |
| Opening selection is already displayed | Minimalism frame 1 contains exactly `cQ0`, the selected `C[Q]`. Replay initializes at step zero, with that step's result visible; there is no preceding empty state. The model selects seven independent items in its first completed stage. Babel derives their display order from future workspace order and puts C first. It did not skip a model-authored first action. | Make the first selection perceptible as an opening action. Treat this as a presentation issue, separately from correctness of the selected syntax; do not invent a terminal under the authored wordless head. |
| Fitting becomes unreadably small in a short desktop window | At 1280×720, the opening C label's rendered box is 7 px high. Final Minimalism and X-bar category boxes reach roughly 5.5 and 5 px. The production component fits into the space above its Replay panel. Nothing being clipped does not make this readable. These dimensions describe the review embedding, not a fresh measurement of the main app's different container. | Establish a readable fit policy for constrained height, preserving the accepted spacious-window appearance and stable stage positions. Do not silently substitute frame-by-frame camera movement. |
| An ordinary merge frame performs no merge | Minimalism frame 25 says `External Merge`, result `T′`, but introduces no visible node; `tBar0` remains hidden until the finite-T relation at frame 26. The structural-step emitter and relation ownership disagree about the same result. | Give each structural change one owner before emitting its micro-step. An ordinary construction step must actually display its result; this does not authorize deleting unchanged relation or Stage Record frames. |
| Future pronunciation leaks backward | Minimalism frames 8–14 mute `Which`; frames 10–14 also mute `book`. Their current authored occurrences are pronounced. A hidden future `whDP0` carries `silent: true`, and inherited-silence readers traverse that layout-only ancestor. The words become green again at frame 15. | Keep future layout context from supplying current linguistic state. Preserve genuine model-authored whole-phrase silence. |
| Five of six movement moments lack a movement drawing | Only Minimalism's John movement, frame 27, recovers movement at its own moment. Minimalism frames 23, 31 and 33 and X-bar frames 37 and 40 remain neutral. The later X-bar licensing relation recovers a path, which does not make its earlier movement moment correct. The causes include feature-sensitive category comparisons, exact-name gates for structural recovery, unfamiliar roles and selecting the base copy instead of the immediately preceding copy. | Use shared, unambiguous occurrence identity and structural evidence, with category annotations handled separately. Keep ambiguity and static-coreference negative controls. Adding five saved-title aliases would leave the underlying problem. |
| The wh transition belongs to the wrong relation | X-bar frame 40 retains the complete object in its old position. Frame 41 introduces the landing and trace during chain licensing, although the model orders movement first. | Assign the structural transition from its owning current/prior evidence independently of whether a later relation gets a better-known drawing. Preserve authored relation order. |
| A fixed camera conceals a moving tree | X-bar 40→41 keeps exactly the same camera transform but repositions unrelated syntax. At 1280×720, `buy` moves about 50 px right and 55 px down; `John` moves about 30 px right and 62 px down. CP also shifts. The 1600×1100 captures reproduce the change. The completed trace replaces a full phrase, changing the layout topology. No other within-stage transition in either DOM walk moved a retained, unchanged label by more than 1 px. | Reserve positions across the actual pre-/post-transition structures, including collapsed traces. Assert coordinates of unchanged syntax, not only the camera matrix. This finding justifies a focused repair, not a general layout rewrite. |
| Explicit multiple Case claims remain neutral | X-bar stage 1 supplies both subject and object Case assignments, each with an assigner, recipient and Case value. The qualified roles and their grouping do not reach the existing drawing. | Investigate general claim grouping and participant/value binding, with ambiguous-group controls. Do not turn every governor or every co-occurring value into a Case claim. |
| Mixed neutral composition is difficult to read | Minimalism's final PF relation creates many participant marks and a detached-looking horizontal rail. X-bar's chain-licensing relation produces both movement and operator-binding curves over the same endpoints. These claims and array participants are retained, but their combined presentation is visually busy. | Review ownership-aware composition using the existing primitives. Do not remove an independent claim merely because endpoints overlap, or invent another linguistic dependency to explain the rail. This is a readability finding, not proven loss or duplication of authored content. |

The source trace reaches shared code rather than sentence-specific branches:

- `movementEvidence.ts` strips primes for category comparison but does not
  separate feature annotations. `C[Q]` differs from `C`; shallow `DP[wh]` fails
  the phrase-label test. The unfamiliar-role structural recovery also has an
  exact relation-identity gate. In a multi-copy chain, a recognized base-copy
  role can win before the explicit prior source is reconciled.
- `replayCompiler.ts` creates structural and relation producers, then rebuilds
  structural steps with pending relation structure hidden. The finite-T step
  survives emission even though its result is withheld.
- Its future-layout skeleton spreads the future node's properties, including
  silence. Pronunciation readers in the compiler and `TreeVisualizer.tsx` walk
  ancestors, allowing an invisible layout ancestor to affect current ink.
- Stage layout uses a shared extent and camera but still lays out distinct
  canvas topologies. A shared fit alone cannot prevent the X-bar trace
  replacement from moving unchanged branches.

The remaining relations were not counted as bugs just because they use Tier 3.
Inventory, selection context, wh features, PF context, locality, licensing and
judgment content remain available. Every relation moment has at least one
available current authored anchor; this pair does **not** establish a need for
a participant-free drawing. Minimalism's internal theta claim recovers a
predication dependency; its external-theta claim supplies an argument and a
predicate domain without an explicit assigner-role pairing. Neither observation
authorizes manufacturing a complete theta grid. X-bar's do-support changes the
head's authored form at its own moment even though the claim remains neutral.
The explicit final grammatical judgment remains readable as authored content.

Several odd-looking details belong to the supplied analyses or settled notation:

- Initial wordless C, silent v, and Minimalism's early `did` are authored.
  Babel must not supply an unrequested null, preterminal or different chronology.
- X-bar explicitly constructs an unfilled CP before wh movement. Its lower
  head is authored as `I-trace` with silent `did`; its final object is authored
  as a childless `DP-trace`. These are not invented gap words or accidentally
  discarded subtrees.
- X-bar's `i`/`j` labels also serve theta-grid references; they are not evidence
  that John and the wh phrase share one movement chain. Circled numbers remain
  stage-scoped relation locators.
- Minimalism still leaves object Case unexplained while its final prose claims
  completion. That is an omission to assess in the model's analysis. Primed
  category notation alongside Bare Phrase Structure prose is a consistency
  question, not by itself proof of an invalid derivation. X-bar's government,
  locality and grammaticality claims are not certified by drawing them.

An offline comparison rebuilt all eight saved analyses, 285 current Replay
frames. Every Sol Stage Record preserves its authored nodes, labels, words,
silence, token indices, lineages and parent-child relations. The six older
Astra/Fable/Grok analyses have neither the new missing construction target nor
the layout-only inherited-silence failure under the same checks. Their two
already-approved deferred top projections remain intentional exceptions to
literal Stage Record visibility, not newly discovered omissions. These checks
do not retrospectively certify all older pixels or linguistic claims.

Progress is therefore mixed. The complete Sol responses, exact final surfaces,
unaltered stages, zero JSON repairs and retained transport evidence improve on
the earlier Fable processing failures and missing evidence. The movement and
short-window Replay experience is still not dependable. A percentage improvement
would have no defensible common denominator.

The verification gap is also concrete: existing controls protect particular
structural arrangements and component behavior, while the first visual pass
checked camera matrices and clipping without comparing unchanged node positions
or the action promised by each frame. `replayScreenshotRegressions.test.mjs`
contains structural assertions; its name does not make it a pixel test. Future
verification should vary feature annotations, prior/intermediate/base copies,
relation-owned construction and full-phrase/trace shapes through the complete
recognition → scheduling → rendering path. That is more useful than another
large collection of literal saved-title tests.

Recommended order: shared movement evidence and transition ownership first;
future-layout isolation and unchanged-node coordinates second; opening/readable
fit and mixed composition review third. The present contract already carries the
evidence needed to investigate these failures. No new contract field or wholesale
redesign is justified by this audit. Finish these offline before requesting
another paid parse. This is a recommendation, not an implemented repair.
