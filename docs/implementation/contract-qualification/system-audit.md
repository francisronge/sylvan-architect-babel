...
**Next connected batch.** Decide the native-plate string formats once (HC05):
they encode list associations the contract already expresses, so the parsers
can be deleted once the pairing sentence covers literal lists. HC21's
traversal-order validation remains. No return to
