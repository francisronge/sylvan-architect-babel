...
test('standalone and Case-composed feature plaques keep row attachment through Replay, zoom, hover and mobile', async () => {
  ...
  try {
    for (const combined of [false, true]) {
      const relations = [...(combined ? [caseRelation] : []), agreement, { relation: 'Another description', anchors: { participant: 'n' } }];
      ...
      const moment = steps.findIndex(step => step.replayRelationIdentity?.relationIndex === Number(combined));
      const next = steps.findIndex(step => step.replayRelationIdentity?.relationIndex === Number(combined) + 1);
      ...
      for (const width of [1200, 390]) {
        ...
        for (const path of state.paths) {
          assert.equal(path.owner, `0:${Number(combined)}`);
        ...
        await go(next);
        const plaque = page.locator('.babel-feature-plaque');
